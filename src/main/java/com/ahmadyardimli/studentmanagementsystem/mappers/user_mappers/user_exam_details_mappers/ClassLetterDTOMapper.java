package com.ahmadyardimli.studentmanagementsystem.mappers.user_mappers.user_exam_details_mappers;

import com.ahmadyardimli.studentmanagementsystem.entities.user_entities.user_exam_details.ClassLetter;
import com.ahmadyardimli.studentmanagementsystem.dtos.user_dtos.user_exam_details.ClassLetterDTO;
import org.springframework.stereotype.Service;

import java.util.function.Function;

@Service
public class ClassLetterDTOMapper implements Function<ClassLetter, ClassLetterDTO> {
    @Override
    public ClassLetterDTO apply(ClassLetter classLetter) {
        return new ClassLetterDTO(
                classLetter.getId(),
                classLetter.getLetterValue()
        );
    }
}
